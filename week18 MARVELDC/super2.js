...
data.forEach((item, index) => {
  ...
  li.className = 'card';
  li.innerHTML = `
	<div class='header'><h2>${item.name}</h2> 
		<span class="tooltip">
			<button class="tooltip-toggle" type="button">
				<img class='click' src='Uf6b997a327074f2d99e2854919fa556an.webp'> 
			</button>
      ...
      <img class='mainImg' src=${item.url}>
    <div class="rating-box">
      <div class='stars' data-card-index="${index}">
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  starsArray.forEach((clickedStarIndex, index) => {
    const stars = document.querySelector(`.stars[data-card-index="${index}"]`).querySelectorAll('i');
...
container.addEventListener('click', event => {
  if (event.target.matches('.stars i')) {
    ...
    const stars = star.parentElement.querySelectorAll('.stars i');
    const clickedStarIndex = Array.from(stars).indexOf(star);
    const cardIndex = star.parentElement.dataset.cardIndex; // Индекс карточки

    starsArray[cardIndex] = clickedStarIndex; // Обновляем или добавляем оценку в массив оценок
    localStorage.setItem('stars', JSON.stringify(starsArray)); // Сохраняем массив оценок в локальное хранилище
    ...
    console.log(`Вы оценили карточку №${cardIndex} на ${clickedStarIndex + 1} звезд(ы)`);
  }
});
